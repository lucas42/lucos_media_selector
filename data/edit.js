{
	...
	$(function (){
		$("tr:not(.static) td:not(.source)").each(makeInput);
	...
	function addRow() {
			...
			var value = $("<td />").addClass("value").each(makeInput);
			var source = $("<td />").addClass("source").text("unsaved");
			$("<tr />").append(label).append(value).append(source).appendTo("table");
